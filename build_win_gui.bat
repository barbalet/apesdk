...
if "%1"=="--debug" (
    set CFLAGS=/Zi /Od /D_WIN32 /DBRAIN_ON /DCOMMON_DEBUG
) else (
    set CFLAGS=/O2 /D_WIN32 /DBRAIN_ON
)
...
REM Compile the longterm.c file
cl %CFLAGS% %COMMANDLINEE% /c win/platform.c

REM Link all object files to create the executable
cl %CFLAGS% %COMMANDLINEE% *.obj /link User32.lib Gdi32.lib ComDlg32.lib /SUBSYSTEM:WINDOWS /OUT:..\guiape.exe
